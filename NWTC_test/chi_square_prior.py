...
#dataset
source_met=os.path.join(cd,'data/met.b0.ASSIST11.nc')
...
bin_hour=np.arange(25)-0.5#bins of hours
...
#load met data
data_met=xr.open_dataset(source_met).rename({'height_therm':'height'})
...
for m in np.unique(month):
    ...
    var=data_met_sel[var_met].where(data_met_sel[var_met]>=np.nanpercentile(data_met_sel[var_met],perc_lim[0]))\
    ...
    #mean
    x_a2=np.nanmean(var,axis=0)
       
    #covariance
    S_a2=np.zeros((len(height),len(height)))
    cov= pd.DataFrame(var[:,h_sel]).dropna().cov().values
    ctr1=0
    for i_h1 in h_sel:
        ctr2=0
        for i_h2 in h_sel:
    ...
    for i_h in h_sel:
        sel=~np.isnan(var[:,i_h])
        x_ah[i_h,:]=binned_statistic(hour[sel],var[sel,i_h],statistic='mean',bins=bin_hour)[0]
    ...
    for i_h in h_sel:
        ...
        plt.plot(data_met_sel.time,var[:,i_h],'-k')
        ...
        plt.hist(var[:,i_h],np.arange(-10,40),density=True,color='k',alpha=0.5)
