...
bin_space=np.arange(0,550,100)#bins of spacing
bin_ws=np.array([0,4,8,12])#bins of wind speed
...
for unit in units:
    ...
    #bin statistics
    f_avg=np.zeros((len(height),len(bin_space)-1,len(bin_Ri)-1,len(bin_ws)-1))
    f_low=np.zeros((len(height),len(bin_space)-1,len(bin_Ri)-1,len(bin_ws)-1))
    f_top=np.zeros((len(height),len(bin_space)-1,len(bin_Ri)-1,len(bin_ws)-1))
    for i_h in range(len(height)):
        i_s=0
        for s1,s2 in zip(bin_space[:-1],bin_space[1:]):
            sel_s=(space_lag.isel(height=i_h)>=s1)*(space_lag.isel(height=i_h)<s2)
            ...
            for Ri1,Ri2 in zip(bin_Ri[:-1],bin_Ri[1:]):
                ...
                for ws1,ws2 in zip(bin_ws[:-1],bin_ws[1:]):
                    sel_ws=(ws.isel(height=i_h)>=ws1)*(ws.isel(height=i_h)<ws2)
                    f_sel=D_T_sel.isel(height=i_h).where(sel_s*sel_Ri*sel_ws).values.ravel()
                    f_avg[i_h,i_s,i_Ri,i_ws]=utl.filt_stat(f_sel,np.nanmean)
                    f_low[i_h,i_s,i_Ri,i_ws]=utl.filt_BS_stat(f_sel,np.nanmean,p_value/2*100)
                    f_top[i_h,i_s,i_Ri,i_ws]=utl.filt_BS_stat(f_sel,np.nanmean,(1-p_value/2)*100)
                    i_ws+=1
                i_Ri+=1
            i_s+=1
        print(f'Structure function at {height[i_h]} m done')
    ...
    # f_std_avg=np.zeros((len(height),len(bin_ws)-1)) 
    # f_std_low=np.zeros((len(height),len(bin_ws)-1)) 
    # f_std_top=np.zeros((len(height),len(bin_ws)-1))        
    # for i_h in range(len(height)):
    #     i_ws=0
    #     for ws1,ws2 in zip(bin_ws[:-1],bin_ws[1:]):
    #         sel_ws=(ws.isel(height=i_h)>=ws1)*(ws.isel(height=i_h)<ws2)
    #         f_sel=diff_sel.isel(height=i_h).where(sel_ws).values
    #         f_std_avg[i_h,i_ws]=np.nanmean(f_sel**2)**0.5
    #         f_std_low[i_h,i_ws]=utl.filt_BS_stat((f_sel**2),np.nanmean,p_value/2*100,perc_lim=[0,100])**0.5
    #         f_std_top[i_h,i_ws]=utl.filt_BS_stat((f_sel**2),np.nanmean,(1-p_value/2)*100,perc_lim=[0,100])**0.5
    #         i_ws+=1
    #     print(f'RMSD at {height[i_h]} m done')
    
    # f_std_avg[np.isnan(f_std_top-f_std_low)]=np.nan
    # std_avg[unit]=xr.DataArray(f_std_avg,coords={'height':height,'ws':(bin_ws[1:]+bin_ws[:-1])/2})
    # std_low[unit]=xr.DataArray(f_std_low,coords={'height':height,'ws':(bin_ws[1:]+bin_ws[:-1])/2})
    # std_top[unit]=xr.DataArray(f_std_top,coords={'height':height,'ws':(bin_ws[1:]+bin_ws[:-1])/2})
    
    f_std_avg=np.zeros((len(height),len(bin_Ri)-1,len(bin_ws)-1))
    f_std_low=np.zeros((len(height),len(bin_Ri)-1,len(bin_ws)-1))
    f_std_top=np.zeros((len(height),len(bin_Ri)-1,len(bin_ws)-1))      
    for i_h in range(len(height)):
        i_Ri=0
        for Ri1,Ri2 in zip(bin_Ri[:-1],bin_Ri[1:]):
            sel_Ri=(Ri>=Ri1)*(Ri<Ri2)
            ...
            for ws1,ws2 in zip(bin_ws[:-1],bin_ws[1:]):
                sel_ws=(ws.isel(height=i_h)>=ws1)*(ws.isel(height=i_h)<ws2)
                f_sel=diff_sel.isel(height=i_h).where(sel_Ri*sel_ws).values
                f_std_avg[i_h,i_Ri,i_ws]=np.nanstd(f_sel)
                f_std_low[i_h,i_Ri,i_ws]=utl.filt_BS_stat(f_sel,np.nanstd,p_value/2*100,perc_lim=[0,100])
                f_std_top[i_h,i_Ri,i_ws]=utl.filt_BS_stat(f_sel,np.nanstd,(1-p_value/2)*100,perc_lim=[0,100])
                i_ws+=1
            i_Ri+=1
        print(f'RMSD at {height[i_h]} m done')
    
    f_std_avg[np.isnan(f_std_top-f_std_low)]=np.nan
    std_avg[unit]=xr.DataArray(f_std_avg,coords={'height':height,'Ri':(bin_Ri[1:]+bin_Ri[:-1])/2,'ws':(bin_ws[1:]+bin_ws[:-1])/2})
    std_low[unit]=xr.DataArray(f_std_low,coords={'height':height,'Ri':(bin_Ri[1:]+bin_Ri[:-1])/2,'ws':(bin_ws[1:]+bin_ws[:-1])/2})
    std_top[unit]=xr.DataArray(f_std_top,coords={'height':height,'Ri':(bin_Ri[1:]+bin_Ri[:-1])/2,'ws':(bin_ws[1:]+bin_ws[:-1])/2})
...
# #             plt.plot(spacing[unit]+(i_h-(len(height)-1)/2)*10,std_avg[unit].isel(height=i_h,ws=i_ws),'^',color=colors[i_h],alpha=0.75)
# #             plt.errorbar(spacing[unit]+(i_h-(len(height)-1)/2)*10,std_avg[unit].isel(height=i_h,ws=i_ws),
# #                                       [[std_avg[unit].isel(height=i_h,ws=i_ws)-std_low[unit].isel(height=i_h,ws=i_ws)],
# #                                        [std_top[unit].isel(height=i_h,ws=i_ws)-std_avg[unit].isel(height=i_h,ws=i_ws)]],
...
#     fig=plt.figure(figsize=(16,5))
#     colors = [cmap(i) for i in np.linspace(0,1,len(height))]
#     for i_Ri in range(len(D_avg[unit].Ri)):
#         ax=plt.subplot(1,len(D_avg[unit].Ri),i_Ri+1)
        
        
#         for i_h in range(len(height)):
#             ax.axvline(spacing[unit],0,1.5,color='k',linestyle='--')
#             plt.plot(D_avg[unit].space,D_avg[unit].isel(height=i_h,Ri=i_Ri),'.-',color=colors[i_h],label=r'$z='+str(int(height[i_h]))+'$ m')
#             ax.fill_between(D_avg[unit].space,D_low[unit].isel(height=i_h,Ri=i_Ri),D_top[unit].isel(height=i_h,Ri=i_Ri),color=colors[i_h],alpha=0.25)
#             plt.plot(spacing[unit]+(i_h-(len(height)-1)/2)*10,std_avg[unit].isel(height=i_h,Ri=i_Ri),'^',color=colors[i_h],alpha=0.75)
#             plt.errorbar(spacing[unit]+(i_h-(len(height)-1)/2)*10,std_avg[unit].isel(height=i_h,Ri=i_Ri),
...
#                                        [std_top[unit].isel(height=i_h,ws=i_ws)-std_avg[unit].isel(height=i_h,ws=i_ws)]],
#                                       color=colors[i_h],capsize=5,alpha=0.75)
#         plt.plot(D_avg[unit_sel].space,D_avg[unit_sel].space**(2/3)*10**-2,'k')
...
S,RI=np.meshgrid(D_avg[unit_sel].space,np.arange(len(D_avg[unit_sel].Ri)))

fig=plt.figure(figsize=(16,5))
for i_h in range(len(height)):
    ax=plt.subplot(1,len(height),i_h+1,projection='3d')
    ax.plot_surface(S,RI,D_avg[unit_sel].isel(height=i_h,ws=2).T,cmap='plasma',linewidth=0)
    # for unit in units:
    #     ax.plot(spacing[unit]+np.zeros(len(std_avg[unit].Ri)),std_avg[unit].ws,std_avg[unit].isel(height=i_h),'.g',markersize=15, zorder=10)
...
plt.figure()
utl.plot_lin_fit(std_avg[unit].values,std_pred[unit].values)