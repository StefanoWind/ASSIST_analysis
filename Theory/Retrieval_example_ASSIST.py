...
for i in range(m):
    plt.semilogx(K[i,:],z,color=colors[i],label=r'$i='+str(i+1)+'$')
for i in range(m):
    plt.plot(ac[i]*np.exp(-1),1/ac[i],'.k',markersize=15,color=colors[i])
plt.xlim([10**-4,1])
plt.xlabel(r'$ \Delta z ~ a_i e^{-a_i z_j}$ [Km$^{-1}$]')
plt.ylabel(r'$z$ [Km]')
plt.grid()
plt.tight_layout()
plt.legend()
...
cbar=fig.colorbar(pc, cax=cbar_ax,label=r'Value of element of $A$',location='left')
