...
for ID in IDs:
    ...
    if len(files_selected)>0:
        ...
        for f in files_selected:
            ...
            T_fnt=np.append(T_fnt,Data['frontEndEnclosureTemp'].values)
            T_abb=np.append(T_abb,Data['ABBapexTemp'].values)
            NEN=np.append(NEN,Data['LWskyNEN'].values)
            
        Output=pd.DataFrame()
        Output['Time']=time
        Output['T_675']=T_675
        Output['T_frontend']=T_fnt
        Output['T_abb']=T_abb
        Output['NEN']=NEN
        
        Output.set_index('Time').to_csv(os.path.join(cd,'data/Summary_T_'+str(ID)+'.csv'))
