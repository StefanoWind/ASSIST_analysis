...
for ID in IDs:
    ...
    sigma_T_diff=(Data['sigma_T_'+str(ID)+'_0.0m']**2+Data['sigma_T_'+str(ID)+'_met']**2)**0.5
    prob_fail[ID]=np.sum(np.abs(DT)>-norm.ppf(p_value/2)*sigma_T_diff)/np.sum(~np.isnan(DT+sigma_T_diff))
...
if plot_all:
    ...
    for t1,t2 in zip(time_bins[:-1],time_bins[1:]):
        ...
        for ID in IDs:
            ...
            plt.fill_between(Data.index[sel], Data['T_'+str(ID)+'_met'].values[sel]-(-norm.ppf(p_value/2))*Data['sigma_T_'+str(ID)+'_met'].values[sel],
                                              Data['T_'+str(ID)+'_met'].values[sel]+(-norm.ppf(p_value/2))*Data['sigma_T_'+str(ID)+'_met'].values[sel],
                                              color='k',alpha=0.25)
    ...
    for t1,t2 in zip(time_bins[:-1],time_bins[1:]):
        ...
        for IDc in ID_comb:
            ...
            sigma_T_diff=(Data['sigma_T_'+str(ID1)+'_met'].values[sel]**2+Data['sigma_T_'+str(ID2)+'_met'].values[sel]**2)**0.5
...
#historgam of temperature differences
